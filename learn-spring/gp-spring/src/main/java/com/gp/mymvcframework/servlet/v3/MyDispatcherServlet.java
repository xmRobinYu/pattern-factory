package com.gp.mymvcframework.servlet.v3;
...
public class MyDispatcherServlet extends HttpServlet {
    ...
    private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        Handler hadle = getHadler(req);
        if (hadle == null) {
            resp.getWriter().write("500 Exection,Detail :  该url 路径不存在 不存在！");
            return;
        }
        Method method = hadle.method;
        ...
        for (Map.Entry<String, String[]> param : params.entrySet()) {
            ...
            //如果找到匹配的对象，则开始填充参数值
            if (!hadle.paramIndexMapping.containsKey(param.getKey())) {
                continue;
            }
            int index = hadle.paramIndexMapping.get(param.getKey());
            paramValues[index] = convert(parameterTypes[index], value);
        }


        //设置方法中的request和response对象
        int reqIndex = hadle.paramIndexMapping.get(HttpServletRequest.class.getName());
        paramValues[reqIndex] = req;
        int respIndex = hadle.paramIndexMapping.get(HttpServletResponse.class.getName());
        paramValues[respIndex] = resp;

        method.invoke(hadle.controller, paramValues);

    }

    private Handler getHadler(HttpServletRequest req) {
    ...
    private void creeateHandleMapping() {
        ...
        for (Object object : iocMap.values()) {
            ...
            for (Method method : clazz.getMethods()) {
                ...
                Handler handler = new Handler(object, method, pattern);
    ...
    public class Handler {
        ...
        public Handler(Object controller, Method method, Pattern pattern) {
            ...
            putInfoParamIndexMapping(method);
        }

        private void putInfoParamIndexMapping(Method method) {
